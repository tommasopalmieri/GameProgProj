...
{
    if ((scene != NULL) && (g_current_scene!=NULL)) scene->lives = g_current_scene->lives;
...
void update()
{
    ...
    g_view_matrix = glm::mat4(1.0f);
        ...
        if (g_current_scene == g_levelB && g_current_scene->m_state.player->get_position().y < -10.0f && g_current_scene->m_state.player->get_position().x > 9.0f)
        {
